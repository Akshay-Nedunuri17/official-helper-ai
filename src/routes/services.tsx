...
function Services() {
  ...
  const cities = useMemo(() => {
    const m = new Map<string, { city: string; state: string; lat: number; lng: number }>();
    offices.forEach((o) => {
      if (o.latitude == null || o.longitude == null) return;
      const key = `${o.city}|${o.state}`;
      if (!m.has(key)) m.set(key, { city: o.city, state: o.state, lat: o.latitude, lng: o.longitude });
    });
    return Array.from(m.values()).sort((a, b) => `${a.state}${a.city}`.localeCompare(`${b.state}${b.city}`));
  }, [offices]);
  ...
  const detectLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      (err) => {
        setLocating(false);
        toast.error(err.code === err.PERMISSION_DENIED ? t("location_permission_denied") : err.message);
        setManualOpen(true);
      },
  ...
  const setManualByCity = (value: string) => {
    const city = cities.find((c) => `${c.city}|${c.state}` === value);
    if (!city) { toast.error(t("pick_city")); return; }
    setUserLoc([city.lat, city.lng]);
    setAccuracy(5000);
    setLocationLabel(`${city.city}, ${city.state}`);
    saveLocation({ lat: city.lat, lng: city.lng, accuracy: 5000, label: `${city.city}, ${city.state}`, source: "manual" });
    setManualOpen(false);
      ...
      <section className="mt-6 rounded-2xl border border-border p-5 gradient-card">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-1">{t("nearest_centers_hint")}</p>
          </div>
        ...
        {manualOpen && (
          <div className="mt-4 rounded-xl border border-border bg-background/70 p-4">
            <div className="flex items-start justify-between gap-3">
              <div>
                <div className="font-semibold text-sm">{t("set_location_manually")}</div>
                <p className="text-xs text-muted-foreground mt-1">{t("manual_location_hint")}</p>
              </div>
            ...
            <select
              className="mt-3 h-10 px-3 rounded-md border border-input bg-background min-w-[18rem] max-w-full"
              defaultValue=""
              onChange={(e) => e.target.value && setManualByCity(e.target.value)}
            >
        ...
        {userLoc && (
          <div className="mt-3 text-xs text-muted-foreground">
            {t("sorted_by_distance")}{locationLabel ? ` · ${locationLabel}` : ""}
            {accuracy != null ? ` · ±${accuracy >= 1000 ? `${(accuracy / 1000).toFixed(1)} km` : `${Math.round(accuracy)} m`}` : ""}
          </div>
        )}

        {!userLoc ? (
          <p className="mt-4 text-sm text-muted-foreground">{t("enable_location_for_nearby")}</p>
        ) : nearestCenters.length === 0 ? (
          <p className="mt-4 text-sm text-muted-foreground">{lang === "en" ? "Loading nearby centers…" : t("nearest_centers")}</p>
        ) : (
          <div className="mt-4 grid lg:grid-cols-[minmax(0,1fr)_360px] gap-4">
